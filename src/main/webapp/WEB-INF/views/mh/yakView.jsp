  ...
  <div class="container">
	...
	<c:forEach items="${yakView}" var="board">
	 <c:if test="${writer == board.unq_num }"><button
			onclick="location.href='/mh/yakModify?pst_num=${board.pst_num}'">수정</button></c:if>
		<button onclick="location.href='/mh/yakList'">목록보기</button>
	 <c:if test="${writer == board.unq_num }"><button onclick="location.href='/mh/yakDelete?pst_num=${board.pst_num}'">삭제</button></c:if>
	</c:forEach>
        </div>
        </div>
        </div>
