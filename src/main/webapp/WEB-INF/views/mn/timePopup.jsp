...
<head>
    ...
    <style type="text/css">
        .reserved { background-color: #323232; pointer-events: none; color: #fdfdfd;}
        .selected { background-color: #daaf67; color: #fdfdfd;}
        ...
        <table class="week_time_table">
            ...
            <tbody>
                <c:forEach var="time" items="${timeList}">
                    <tr>
                        <th class="col_1">${time}</th>
                        <c:forEach var="day" items="${dayList}">
                            <td data-day="${day}" data-time="${time}">${time }</td>
                        </c:forEach>
                    </tr>
                </c:forEach>
            </tbody>
        </table>
        <button id="completeButton" class="simbut">완료</button>
    </div>
