...
public class HsDaoImpl implements HsDao {
	...
	// 해당 강의 수강하는 학생 수 count
	@Override
	public int getLecCount(Hs_Assignment hsAss) {
		System.out.println("HsDaoImpl getLecCount Start...");
		int count = 0;
		try {
			count = session.selectOne("hslecTotal", hsAss);
			System.out.println("HsDaoImpl getLecCount count-> "+count);
		} catch (Exception e) {
			System.out.println("HsDaoImpl getLecCount e.getMessage() ->"+ e.getMessage());
			e.printStackTrace();
		}
		return count;
	}
